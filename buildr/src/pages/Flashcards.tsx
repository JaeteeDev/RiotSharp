  ...
  if (!deck.length) {
    return (
      <div className="mx-auto max-w-lg px-8 py-20 text-center">
        <h2 className="font-display text-xl font-semibold text-paper-100">No cards in this deck yet</h2>
        <p className="mt-2 text-[13px] text-mute-500">Try a different filter, or complete a few quizzes first so BUILDR can build a "missed questions" deck.</p>
      </div>
    )
  }

  return (
    <div className="mx-auto flex max-w-3xl flex-col items-center px-8 py-8">
      <div className="mb-6 flex flex-wrap items-center justify-center gap-2">
        ...
        {filter === 'area' && (
          <select value={areaId} onChange={(e) => setAreaId(e.target.value)} className="rounded-md border border-ink-600 bg-ink-850 px-2.5 py-1.5 text-[12px] text-paper-200">
            {learningAreas.map((a) => (
      ...
      <div className="text-technical mb-3 text-[11px] uppercase tracking-wide text-mute-500">
        Card {(index % deck.length) + 1} / {deck.length} · {sessionStats.correct} correct · {sessionStats.incorrect} to review
      </div>

      <div className="[perspective:1400px]" style={{ width: 460, height: 300 }}>
        <motion.div
          key={card.id}
        ...
        >
          <div className="absolute inset-0 flex flex-col items-center justify-center rounded-xl border border-ink-600 bg-ink-850 p-8 text-center [backface-visibility:hidden]">
            <span className="text-technical mb-4 text-[10px] uppercase tracking-wide text-mute-500">Front</span>
            <span className="font-display text-2xl font-semibold tracking-wide text-paper-100">{card.front}</span>
          </div>
          <div
            className="absolute inset-0 flex flex-col items-center justify-center rounded-xl border border-signal-500/40 bg-ink-800 p-8 text-center [backface-visibility:hidden]"
            style={{ transform: 'rotateY(180deg)' }}
          >
            <span className="text-technical mb-3 text-[10px] uppercase tracking-wide text-signal-400">Back</span>
            <p className="text-[13.5px] leading-relaxed text-paper-200">{card.back}</p>
          </div>
        </motion.div>
      </div>
      ...
      <div className="mt-6 flex items-center gap-3">
        <button
          onClick={() => next(false)}
          disabled={!flipped}
          className="flex items-center gap-1.5 rounded-md border border-bad-400/40 px-4 py-2.5 text-[13px] text-bad-400 transition-colors disabled:opacity-30"
        >
          <ArrowLeft className="h-3.5 w-3.5" /> Incorrect
        </button>
        <button
          onClick={() => setFlipped((f) => !f)}
          className="flex items-center gap-1.5 rounded-md border border-ink-500 px-4 py-2.5 text-[13px] text-paper-200"
        >
        ...
        <button
          onClick={() => next(true)}
          disabled={!flipped}
          className="flex items-center gap-1.5 rounded-md border border-good-400/40 px-4 py-2.5 text-[13px] text-good-400 transition-colors disabled:opacity-30"
        >
