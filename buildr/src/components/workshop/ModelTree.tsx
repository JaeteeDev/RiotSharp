...
import { wallComponents, wallGroupLabels, type WallComponentGroup } from '../../data/wallFrame'
...
export function ModelTree({
  ...
  return (
    <div className="flex flex-col gap-0.5 overflow-y-auto py-2">
      <div className="text-technical px-3 pb-2 text-[10px] uppercase tracking-wide text-mute-500">Wall Frame 01</div>
        ...
        return (
          <div key={group}>
            <button
              ...
              className="flex w-full items-center gap-1.5 px-3 py-1.5 text-left text-[12px] font-medium text-paper-300 hover:text-paper-100"
            >
              <TreeChevron collapsed={isCollapsed} />
              {wallGroupLabels[group]}
              <span className="text-technical ml-auto text-[10px] text-mute-600">{items.length}</span>
            </button>
            {!isCollapsed &&
              items.map((c) => (
                <div
                  ...
                  className={clsx(
                    'group flex items-center gap-2 py-1 pl-8 pr-3 text-[12px] transition-colors',
